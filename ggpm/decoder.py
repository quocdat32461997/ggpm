...
class HierMPNDecoder(nn.Module):
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        for t in range(maxt):  # loop over max generation
            ...
            # apply mask to get current tree
            ...
            htree, hinter, hgraph = self.hmpn(cur_tree_tensors, cur_inter_tensors, htree, hinter, subtree)
...
class MotifDecoder(torch.nn.Module):
    ...
    def apply_tree_mask(self, tensors, cur):
        # Get current tree
        fnode, fmess, agraph, bgraph, cgraph, scope = tensors
        agraph = agraph * index_select_ND(cur.emask, 0, agraph)
        bgraph = bgraph * index_select_ND(cur.emask, 0, bgraph)
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        for t in range(maxt):
            ...
            # get subtree (aka ROOT)
            subtree = [], [] # tuple of subtrees and edges between consecutive subtrees
            for i in batch_list:
            ...
            htree = self.hmpn(cur_tree_tensors, cur_inter_tensors, htree, subtree)

            new_atoms = []
            for i in batch_list:
                # get topology labels upto t step
                xid, yid, tlab = orders[i][t]
                all_topo_preds.append((htree.node[xid], i, tlab))  # topology prediction
                if yid is not None:
                    mess_idx = tree_batch[xid][yid]['mess_idx']
                ...
                # get labels (motif, attachment) at t step
                # and predict motif and attachment at t step
                cls = tree_batch.nodes[yid]['smiles']
                clab, ilab = self.vocab[tree_batch.nodes[yid]['label']]
                mess_idx = tree_batch[xid][yid]['mess_idx']
                hmess = self.rnn_cell.get_hidden_state(htree.mess)
